<template>
  <div class="page-container">
      ...
      <el-table :data="tableData" stripe style="width: 100%" v-loading="tableLoading">
        ...
        <el-table-column prop="formalStatus" label="转正状态" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="statusTag(row.formalStatus)" size="small">{{ statusMap[row.formalStatus] }}</el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 审核对话框 -->
    <el-dialog v-model="auditDialog" title="转正审核" width="500px" :close-on-click-modal="false" destroy-on-close>
    ...
    <!-- 驳回对话框 -->
    <el-dialog v-model="rejectDialog" title="驳回转正申请" width="450px" :close-on-click-modal="false" destroy-on-close>
...
const submitAudit = async () => {
  ...
  try {
    await formalAudit(currentRow.value.id, { auditLevel: auditLevel.value, status: auditForm.status, auditRemark: auditForm.auditRemark })
...
const submitReject = async () => {
  ...
  try {
    await rejectOrder(currentRow.value.id, { auditPhase: 'FORMAL', rejectReason: rejectReason.value })
